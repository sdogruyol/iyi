...
case "$(uname -s)" in
  Linux)
    ...
    ;;
  *) allowed_symbols="__error _tlv_bootstrap madvise pipe pthread_create pthread_kill sigaction sysctlbyname read _dyld_get_image_header _dyld_get_image_vmaddr_slide clock_gettime clock_gettime_nsec_np exit kevent kqueue mmap munmap pthread_get_stackaddr_np pthread_self write" ;;
esac
