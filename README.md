...
current compiler reports `0.10.0`. iyi's own prelude carries the small-tool
...
it uses, and iyi's own library is 10,038 lines rather than 8,161. The whole
library is 406 KB on disk beside the binary.
...
The tarball is relocatable and carries both libraries: iyi's own 406 KB, and
...
program *has*: 8,161 lines of Crystal's standard library instead of 10,038
...
**Is the syntax stable?** No. 0.10.0 is a release of a language that is still
...
language rather than the library — master is 0.10.0, and the parts of SPEC.md
marked PROPOSED are the parts that will move under you.

## What is not here

- **iyi's own library is 10,038 lines, and its IO is `puts`, `print`,
...
- **Artifacts are identified by released version, target and flag set.** Builds
  ...
  commit in its identity and interoperates only with itself. 0.10.0 is
  released, so its artifacts carry the version alone and any 0.10.0 build on
...
| [`src/iyi`](src/iyi) | iyi's own library, 10,038 lines. `--crystal` swaps it for Crystal's |
...
`iyi 0.10.0 (built on Crystal 1.22.0-dev)`: the language first, then what it
