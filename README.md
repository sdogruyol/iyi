...
current compiler reports `0.7.0`. iyi's own prelude carries the small-tool
...
The released tarball is 0.7.0, and a build from current source reports the
same.

```console
$ tar -xzf iyi-0.7.0-linux-x86_64.tar.gz -C ~/.local
...
**Is the syntax stable?** No. 0.7.0 is a release of a language that is still
...
language rather than the library — master is 0.7.0, and the parts of SPEC.md
...
- **Artifacts are identified by released version, target and flag set.** Builds
  ...
  commit in its identity and interoperates only with itself. 0.7.0 is
  released, so its artifacts carry the version alone and any 0.7.0 build on
...
`iyi 0.7.0 (built on Crystal 1.22.0-dev)`: the language first, then what it
