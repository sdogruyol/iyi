...
| iyi's own prelude | 8,878 lines, ceiling 3,734 |
...
> "No standard library worth the name" is still true of iyi's own 8,878 lines
...
**3. A deliberately tiny prelude, written in iyi. Done: 8,878 lines,
...
|---|---|---|
| Compiler | 24,984 lines, **written in Crystal** | 103,338 lines, Crystal, forked |
| Library | 8,161 lines (3,551 of it core) | 8,878-line own prelude + 778 in samples |
| Specs | 21,146 lines | 9,360 for iyi |
...
sweep left the pause and is the allocator's, an arena at a time outside
the runtime lock; the mark is parallel on helper threads and runs
beside the program on a write barrier the compiler emits, a collection
being two stops of tens of microseconds; the header is one word, the
classes eight bytes apart, and the sweep hands pages back to the
kernel; a mutator outrunning the mark allocates black and assists. The
price today: 26 ns an allocation with one thread, 63 with four, 162
with eight, and `bench/gc_race.py`'s table beside Boehm and Go — the
pauses Go's or under on every program, the footprint Go's on churn and
within a budget of it on the rest.
...
12. ~~**Using Crystal code.**~~ **Specified in III.6; the direct source mode
    ...
    **The Crystal ecosystem, and what a shard would cost.** Ten thousand
    ...
    Crystal's 8,161-line standard library against iyi's own 8,878-line prelude.
